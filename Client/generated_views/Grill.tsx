...
export function Grill_Grill_Recipe_can_create(self:GrillContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanCreate
}
export function Grill_Grill_Recipe_can_delete(self:GrillContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanDelete
}
export function Grill_Grill_Recipe_page_index(self:GrillContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 0 : state.Recipe.PageIndex
}
export function Grill_Grill_Recipe_page_size(self:GrillContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 25 : state.Recipe.PageSize
}
export function Grill_Grill_Recipe_num_pages(self:GrillContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 1 : state.Recipe.NumPages
}

export function load_relation_Grill_Grill_Recipe(self:GrillContext, current_User:Models.User, callback?:()=>void) {
  Permissions.can_view_Recipe(current_User) ?
    Api.get_Grill_Grill_Recipes(self.props.entity, Grill_Grill_Recipe_page_index(self), Grill_Grill_Recipe_page_size(self)).then(Recipes =>
...
export function load_relations_Grill(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Grill_Grill_Recipe(self, self.props.current_User, 
        () => callback && callback())
}
...
export function render_local_menu_Grill(self:GrillContext) {
  ...
  return <div className="local-menu">
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Recipe(self.props.current_User) ? null :
                  <div key={"Grill_Recipe"} className={`local_menu_entry${self.props.shown_relation == "Grill_Recipe" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Grill_Grill_Recipe(self,
                        self.props.current_User, 
                        () => self.props.set_shown_relation("Grill_Recipe"))
                    }>
                      {i18next.t('Grill_Recipes')}
                    </a>
                  </div>
                }  
          </div>
...
export function render_controls_Grill(self:GrillContext) {
  return <div className="control">
    ...
    {Permissions.can_delete_Grill(self.props.current_User) && self.props.size == "fullscreen" ? <a className="button button--delete"
...
export function render_content_Grill(self:GrillContext) {
  return <div className={`${self.props.inline != undefined && self.props.inline ? "" : "model-content"} ${self.props.size == 'preview' ? 'model-content--preview' : ''}`}>
    {Permissions.can_view_Grill(self.props.current_User) ?
...
export function render_Grill_Grill_Recipe(self:GrillContext, context:"presentation_structure"|"default") {
  if ((context == "default" && self.props.shown_relation != "all" && self.props.shown_relation != "Grill_Recipe") || !Permissions.can_view_Recipe(self.props.current_User))
    return null
  let state = self.state()
  return <div>
    { List.render_relation("grill_grill_recipe",
   "Grill",
   "Recipe",
   "Recipes",
  ...
  (
      ...
      Grill_Grill_Recipe_page_index(self),
      Grill_Grill_Recipe_num_pages(self),
      new_page_index => {
          let state = self.state()
            ...
            <div key={i_id}>
              {
                RecipeViews.Recipe({
                  ...
                  unlink: !Permissions.can_delete_Grill_Recipe(self.props.current_User) ?
                    null
                    :
                    () => confirm(i18next.t('Are you sure?')) && Api.unlink_Grill_Grill_Recipes(self.props.entity, i.element).then(() =>
                      load_relation_Grill_Grill_Recipe(self, self.props.current_User))
                })
              }
            </div>
      ...
      () =>
        <div>
          {Permissions.can_create_Recipe(self.props.current_User) && Permissions.can_create_Grill_Recipe(self.props.current_User) && Grill_Grill_Recipe_can_create(self) ? render_new_Grill_Grill_Recipe(self) : null}
          {Permissions.can_create_Grill_Recipe(self.props.current_User) ? render_add_existing_Grill_Grill_Recipe(self) : null}
        </div>)
    }
...
export function render_relations_Grill(self:GrillContext) {
  return <div className="relations">
      { render_Grill_Grill_Recipe(self, "default") }
      
    </div>
}

export function render_add_existing_Grill_Grill_Recipe(self:GrillContext) {
      ...
      <div className="button__actions">
        {
          ...
          React.createElement(List.AddToRelation,
            {
              relation_name:"grill_grill_recipe",
              source_name:"Grill",
              target_name:"Recipe",
                  ...
                  <a className="group__button button button--existing"
                    onClick={() =>
                        self.setState({...self.state(), add_step_Recipe:"saving"}, () =>
                          Api.link_Grill_Grill_Recipes(self.props.entity, i).then(() =>
              ...
              get_items:[
                { name: "Recipe", get: async(i,s) => Api.get_unlinked_Grill_Grill_Recipes(self.props.entity, i, s) },
              ]
            })
        }
      </div>
...
export function render_new_Grill_Grill_Recipe(self:GrillContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
        <div className="new-recipe">
              <button 
                      ...
                      onClick={() =>
                          Api.create_linked_Grill_Grill_Recipes_Recipe(self.props.entity).then(e => {
                              e.length > 0 &&
                              Api.update_Recipe(
...
export let Grill_to_page = (id:number) => {
  let can_edit = Utils.any_of([Permissions.can_edit_Grill, Permissions.can_edit_Grill_Recipe, Permissions.can_edit_Recipe])
  return Utils.scene_to_page<Models.Grill>(can_edit, Grill, Api.get_Grill(id), Api.update_Grill, "Grill", "Grill", `/Grills/${id}`)
}

export let Grill_to = (id:number, target_element_id:string, current_User:Models.User) => {
