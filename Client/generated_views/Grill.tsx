...
export function load_relation_Grill_Cuisine_Meal(self:GrillContext, force_first_page:boolean, current_User:Models.User, callback?:()=>void) {
  ...
  Permissions.can_view_Meal(current_User) ?
    prelude(() =>
      Api.get_Cuisine_Cuisine_Meals(self.props.entity, Grill_Cuisine_Meal_page_index(self), Grill_Cuisine_Meal_page_size(self), Grill_Cuisine_Meal_search_query(self)).then(Meals =>
...
export function render_Grill_Description_editable_minimised(self:GrillContext) : JSX.Element {
  if (!Permissions.can_edit_Grill(self.props.current_User)) return render_Grill_Description_minimised(self)
  else
    return !Permissions.can_view_Grill_Description(self.props.current_User) ? <div /> :
          <div className="model__attribute description">
  <label className="attribute-label attribute-label-description">{i18next.t(`Grill:Description`, {context: self.props.inline ? "inline" : ""})}</label>
  <div className="model__attribute-content">
    { Components.String(
        self.props.is_editable && Permissions.can_edit_Grill(self.props.current_User) && Permissions.can_edit_Grill_Description(self.props.current_User),
        self.props.mode,
...
export function render_Grill_Description_editable_maximised(self:GrillContext) : JSX.Element {
  if (!Permissions.can_edit_Grill(self.props.current_User)) return render_Grill_Description_maximised(self)
  else
    return !Permissions.can_view_Grill_Description(self.props.current_User) ? <div /> :
          <div className="model__attribute description">
  <label className="attribute-label attribute-label-description">{i18next.t(`Grill:Description`, {context: self.props.inline ? "inline" : ""})}</label>
...
export function render_editable_attributes_minimised_Grill(self:GrillContext) {
  let attributes = (<div>
      {render_Grill_Description_editable_minimised(self)}
    </div>)
  return attributes
}
...
export function render_editable_attributes_maximised_Grill(self:GrillContext) {
    let state = self.state()
    let attributes = (<div>
        {render_Grill_Description_editable_maximised(self)}
          ...
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Meal(self.props.current_User) ? null :
                  <div key={"Cuisine_Meal"} className={`local_menu_entry${self.props.shown_relation == "Cuisine_Meal" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Grill_Cuisine_Meal(self,
                        false,
...
export function render_Grill_Description_minimised(self:GrillContext) : JSX.Element {
      return !Permissions.can_view_Grill_Description(self.props.current_User) ? null : <div className="model__attribute description">
  <label className="attribute-label attribute-label-description">{i18next.t(`Grill:Description`, {context: self.props.inline ? "inline" : ""})}</label>
  <div className="model__attribute-content">
    { Components.String(
        self.props.is_editable && Permissions.can_edit_Grill(self.props.current_User) && Permissions.can_edit_Grill_Description(self.props.current_User),
...
export function render_Grill_Description_maximised(self:GrillContext) : JSX.Element {
        return !Permissions.can_view_Grill_Description(self.props.current_User) ? null : <div className="model__attribute description">
  <label className="attribute-label attribute-label-description">{i18next.t(`Grill:Description`, {context: self.props.inline ? "inline" : ""})}</label>
  <div className="model__attribute-content">
    { Components.String(
...
export function render_preview_Grill(self:GrillContext) {
  ...
  if (self.props.mode == "view" || !Permissions.can_edit_Grill(self.props.current_User))
    attributes = (<div className="model__attributes">
      { render_Grill_Description_minimised(self) }
    </div>)
  else
...
export function render_large_Grill(self:GrillContext) {
  ...
  if (self.props.mode == "view" || !Permissions.can_edit_Grill(self.props.current_User))
    attributes = (<div className="model__attributes">
      { render_Grill_Description_maximised(self) }
...
export function render_Grill_Cuisine_Meal(self:GrillContext, context:"presentation_structure"|"default") {
  ...
  return <div>
    ...
    { List.render_relation("grill_cuisine_meal",
   "Cuisine",
   "Meal",
   "Meals",
  ...
  (
      state.Meal != "loading" ?
        ...
        state.Meal,
      Grill_Cuisine_Meal_page_index(self),
      Grill_Cuisine_Meal_num_pages(self),
      new_page_index => {
          let state = self.state()
          state.Meal != "loading" &&
...
export function render_add_existing_Grill_Cuisine_Meal(self:GrillContext) {
...
export function render_new_Grill_Cuisine_Meal(self:GrillContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
                      ...
                      onClick={() =>
                          Api.create_linked_Cuisine_Cuisine_Meals_Lunch(self.props.entity).then(e => {
                              ...
                              Api.update_Lunch(
                                ...
                                load_relation_Grill_Cuisine_Meal(self, true, self.props.current_User, () =>
                                    self.setState({...self.state(), add_step_Meal:"closed"})
                                  )
                                )
                          })
