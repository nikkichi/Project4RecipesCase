...
export function Mediterranean_Homepage_Cuisine_can_create(self:MediterraneanContext) {
  let state = self.state()
...
export function Mediterranean_Homepage_Cuisine_can_delete(self:MediterraneanContext) {
  let state = self.state()
...
export function Mediterranean_Homepage_Cuisine_page_index(self:MediterraneanContext) {
  let state = self.state()
...
export function Mediterranean_Homepage_Cuisine_page_size(self:MediterraneanContext) {
  let state = self.state()
...
export function Mediterranean_Homepage_Cuisine_search_query(self:MediterraneanContext) {
  let state = self.state()
...
export function Mediterranean_Homepage_Cuisine_num_pages(self:MediterraneanContext) {
  let state = self.state()
...
export function load_relation_Mediterranean_Homepage_Cuisine(self:MediterraneanContext, force_first_page:boolean, current_User:Models.User, callback?:()=>void) {
  let state = self.state()
  ...
  Permissions.can_view_Homepage(current_User) ?
    prelude(() =>
      Api.get_Cuisine_Homepage_Cuisines(self.props.entity, Mediterranean_Homepage_Cuisine_page_index(self), Mediterranean_Homepage_Cuisine_page_size(self), Mediterranean_Homepage_Cuisine_search_query(self)).then(Homepages =>
        self.setState({...self.state(), update_count:self.state().update_count+1,
...
export function load_relations_Mediterranean(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Mediterranean_Homepage_Cuisine(self, false, self.props.current_User, 
...
export function render_Mediterranean_Homepage_Cuisine(self:MediterraneanContext, context:"presentation_structure"|"default") {
  ...
  return <div>
    ...
    { List.render_relation("mediterranean_homepage_cuisine",
   "Cuisine",
   "Homepage",
   "Homepages",
   self.props.nesting_depth > 0,
  ...
  (
      state.Homepage != "loading" ?
        ...
        state.Homepage,
      Mediterranean_Homepage_Cuisine_page_index(self),
      Mediterranean_Homepage_Cuisine_num_pages(self),
      new_page_index => {
          let state = self.state()
          state.Homepage != "loading" &&
          self.setState({...self.state(),
            ...
            <div key={i_id}>
              {
                HomepageViews.Homepage({
                  ...
                  unlink: undefined,
                    delete: !Permissions.can_delete_Homepage(self.props.current_User) || !Mediterranean_Homepage_Cuisine_can_delete(self) ?
                    null
                    :
...
export let Mediterranean_to_page = (id:number) => {
  let can_edit = Utils.any_of([Permissions.can_edit_Mediterranean, Permissions.can_edit_Cuisine_Meal, Permissions.can_edit_Homepage_Cuisine, Permissions.can_edit_Meal, Permissions.can_edit_Homepage])
