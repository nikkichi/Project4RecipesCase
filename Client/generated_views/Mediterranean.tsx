...
export function Mediterranean_Mediterranean_Recipe_can_create(self:MediterraneanContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanCreate
}
export function Mediterranean_Mediterranean_Recipe_can_delete(self:MediterraneanContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanDelete
}
export function Mediterranean_Mediterranean_Recipe_page_index(self:MediterraneanContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 0 : state.Recipe.PageIndex
}
export function Mediterranean_Mediterranean_Recipe_page_size(self:MediterraneanContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 25 : state.Recipe.PageSize
}
export function Mediterranean_Mediterranean_Recipe_num_pages(self:MediterraneanContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 1 : state.Recipe.NumPages
}

export function load_relation_Mediterranean_Mediterranean_Recipe(self:MediterraneanContext, current_User:Models.User, callback?:()=>void) {
  Permissions.can_view_Recipe(current_User) ?
    Api.get_Mediterranean_Mediterranean_Recipes(self.props.entity, Mediterranean_Mediterranean_Recipe_page_index(self), Mediterranean_Mediterranean_Recipe_page_size(self)).then(Recipes =>
...
export function load_relations_Mediterranean(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Mediterranean_Mediterranean_Recipe(self, self.props.current_User, 
        () => callback && callback())
}
...
export function render_local_menu_Mediterranean(self:MediterraneanContext) {
  ...
  return <div className="local-menu">
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Recipe(self.props.current_User) ? null :
                  <div key={"Mediterranean_Recipe"} className={`local_menu_entry${self.props.shown_relation == "Mediterranean_Recipe" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Mediterranean_Mediterranean_Recipe(self,
                        self.props.current_User, 
                        () => self.props.set_shown_relation("Mediterranean_Recipe"))
                    }>
                      {i18next.t('Mediterranean_Recipes')}
                    </a>
                  </div>
                }  
          </div>
...
export function render_Mediterranean_Mediterranean_Recipe(self:MediterraneanContext, context:"presentation_structure"|"default") {
  if ((context == "default" && self.props.shown_relation != "all" && self.props.shown_relation != "Mediterranean_Recipe") || !Permissions.can_view_Recipe(self.props.current_User))
    return null
  let state = self.state()
  return <div>
    { List.render_relation("mediterranean_mediterranean_recipe",
   "Mediterranean",
   "Recipe",
   "Recipes",
  ...
  (
      ...
      Mediterranean_Mediterranean_Recipe_page_index(self),
      Mediterranean_Mediterranean_Recipe_num_pages(self),
      new_page_index => {
          let state = self.state()
            ...
            <div key={i_id}>
              {
                RecipeViews.Recipe({
                  ...
                  unlink: !Permissions.can_delete_Mediterranean_Recipe(self.props.current_User) ?
                    null
                    :
                    () => confirm(i18next.t('Are you sure?')) && Api.unlink_Mediterranean_Mediterranean_Recipes(self.props.entity, i.element).then(() =>
                      load_relation_Mediterranean_Mediterranean_Recipe(self, self.props.current_User))
                })
              }
            </div>
      ...
      () =>
        <div>
          {Permissions.can_create_Recipe(self.props.current_User) && Permissions.can_create_Mediterranean_Recipe(self.props.current_User) && Mediterranean_Mediterranean_Recipe_can_create(self) ? render_new_Mediterranean_Mediterranean_Recipe(self) : null}
          {Permissions.can_create_Mediterranean_Recipe(self.props.current_User) ? render_add_existing_Mediterranean_Mediterranean_Recipe(self) : null}
        </div>)
    }
...
export function render_relations_Mediterranean(self:MediterraneanContext) {
  return <div className="relations">
      { render_Mediterranean_Mediterranean_Recipe(self, "default") }
      
    </div>
}

export function render_add_existing_Mediterranean_Mediterranean_Recipe(self:MediterraneanContext) {
      ...
      <div className="button__actions">
        {
          ...
          React.createElement(List.AddToRelation,
            {
              relation_name:"mediterranean_mediterranean_recipe",
              source_name:"Mediterranean",
              target_name:"Recipe",
                  ...
                  <a className="group__button button button--existing"
                    onClick={() =>
                        self.setState({...self.state(), add_step_Recipe:"saving"}, () =>
                          Api.link_Mediterranean_Mediterranean_Recipes(self.props.entity, i).then(() =>
              ...
              get_items:[
                { name: "Recipe", get: async(i,s) => Api.get_unlinked_Mediterranean_Mediterranean_Recipes(self.props.entity, i, s) },
              ]
            })
        }
      </div>
...
export function render_new_Mediterranean_Mediterranean_Recipe(self:MediterraneanContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
        <div className="new-recipe">
              <button 
                      ...
                      onClick={() =>
                          Api.create_linked_Mediterranean_Mediterranean_Recipes_Recipe(self.props.entity).then(e => {
                              e.length > 0 &&
                              Api.update_Recipe(
...
export let Mediterranean_to_page = (id:number) => {
  let can_edit = Utils.any_of([Permissions.can_edit_Mediterranean, Permissions.can_edit_Mediterranean_Recipe, Permissions.can_edit_Recipe])
