...
export function Rating_Rating_Recipe_can_create(self:RatingContext) {
  let state = self.state()
...
export function Rating_Rating_Recipe_can_delete(self:RatingContext) {
  let state = self.state()
...
export function Rating_Rating_Recipe_page_index(self:RatingContext) {
  let state = self.state()
...
export function Rating_Rating_Recipe_page_size(self:RatingContext) {
  let state = self.state()
...
export function Rating_Rating_Recipe_num_pages(self:RatingContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 1 : state.Recipe.NumPages
}

export function load_relation_Rating_User_Rating(self:RatingContext, current_User:Models.User, callback?:()=>void) {
  Permissions.can_view_User(current_User) ?
    Api.get_Rating_User_Ratings(self.props.entity, Rating_User_Rating_page_index(self), Rating_User_Rating_page_size(self)).then(Users =>
...
export function load_relation_Rating_Rating_Recipe(self:RatingContext, current_User:Models.User, callback?:()=>void) {
  Permissions.can_view_Recipe(current_User) ?
    Api.get_Rating_Rating_Recipes(self.props.entity, Rating_Rating_Recipe_page_index(self), Rating_Rating_Recipe_page_size(self)).then(Recipes =>
      self.setState({...self.state(), update_count:self.state().update_count+1,
...
export function load_relations_Rating(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Rating_Rating_Recipe(self, self.props.current_User, 
        () => load_relation_Rating_User_Rating(self, self.props.current_User, 
        () => callback && callback()))
}
...
export function render_local_menu_Rating(self:RatingContext) {
  ...
  return <div className="local-menu">
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Recipe(self.props.current_User) ? null :
                  <div key={"Rating_Recipe"} className={`local_menu_entry${self.props.shown_relation == "Rating_Recipe" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Rating_Rating_Recipe(self,
                        self.props.current_User, 
                        () => self.props.set_shown_relation("Rating_Recipe"))
                    }>
                      {i18next.t('Rating_Recipes')}
                    </a>
                  </div>
                }  
          </div>
...
export function render_Rating_Rating_Recipe(self:RatingContext, context:"presentation_structure"|"default") {
  if ((context == "default" && self.props.shown_relation != "all" && self.props.shown_relation != "Rating_Recipe") || !Permissions.can_view_Recipe(self.props.current_User))
    return null
  let state = self.state()
  return <div>
    { List.render_relation("rating_rating_recipe",
   "Rating",
   "Recipe",
   "Recipes",
   self.props.nesting_depth > 0,
  ...
  (
      ...
      Rating_Rating_Recipe_page_index(self),
      Rating_Rating_Recipe_num_pages(self),
      new_page_index => {
          let state = self.state()
            ...
            <div key={i_id}>
              {
                RecipeViews.Recipe({
                  ...
                  unlink: !Permissions.can_delete_Rating_Recipe(self.props.current_User) ?
                    null
                    :
                    () => confirm(i18next.t('Are you sure?')) && Api.unlink_Rating_Rating_Recipes(self.props.entity, i.element).then(() =>
                      load_relation_Rating_Rating_Recipe(self, self.props.current_User))
                })
              }
            </div>
      ...
      () =>
        <div>
          {Permissions.can_create_Recipe(self.props.current_User) && Permissions.can_create_Rating_Recipe(self.props.current_User) && Rating_Rating_Recipe_can_create(self) ? render_new_Rating_Rating_Recipe(self) : null}
          {Permissions.can_create_Rating_Recipe(self.props.current_User) ? render_add_existing_Rating_Rating_Recipe(self) : null}
        </div>)
    }
...
export function render_relations_Rating(self:RatingContext) {
  return <div className="relations">
      { render_Rating_Rating_Recipe(self, "default") }
...
export function render_add_existing_Rating_Rating_Recipe(self:RatingContext) {
      ...
      <div className="button__actions">
        {
          ...
          React.createElement(List.AddToRelation,
            {
              relation_name:"rating_rating_recipe",
              source_name:"Rating",
              target_name:"Recipe",
                  ...
                  <a className="group__button button button--existing"
                    onClick={() =>
                        self.setState({...self.state(), add_step_Recipe:"saving"}, () =>
                          Api.link_Rating_Rating_Recipes(self.props.entity, i).then(() =>
              ...
              get_items:[
                { name: "Recipe", get: async(i,s) => Api.get_unlinked_Rating_Rating_Recipes(self.props.entity, i, s) },
              ]
            })
        }
      </div>
...
export function render_new_Rating_Rating_Recipe(self:RatingContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
        <div className="new-recipe">
              <button 
                      ...
                      onClick={() =>
                          Api.create_linked_Rating_Rating_Recipes_Recipe(self.props.entity).then(e => {
                              e.length > 0 &&
                              Api.update_Recipe(
...
export let Rating_to_page = (id:number) => {
  let can_edit = Utils.any_of([Permissions.can_edit_Rating, Permissions.can_edit_User_Rating, Permissions.can_edit_Rating_Recipe, Permissions.can_edit_User, Permissions.can_edit_Recipe])
