...
export function Thirty_PreparationTime_Recipe_can_create(self:ThirtyContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanCreate
}
export function Thirty_PreparationTime_Recipe_can_delete(self:ThirtyContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanDelete
}
export function Thirty_PreparationTime_Recipe_page_index(self:ThirtyContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 0 : state.Recipe.PageIndex
}
export function Thirty_PreparationTime_Recipe_page_size(self:ThirtyContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 25 : state.Recipe.PageSize
}
export function Thirty_PreparationTime_Recipe_search_query(self:ThirtyContext) {
  let state = self.state()
  return state.Recipe == "loading" ? null : state.Recipe.SearchQuery
}
export function Thirty_PreparationTime_Recipe_num_pages(self:ThirtyContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 1 : state.Recipe.NumPages
}

export function load_relation_Thirty_PreparationTime_Recipe(self:ThirtyContext, force_first_page:boolean, current_User:Models.User, callback?:()=>void) {
  let state = self.state()
  ...
  Permissions.can_view_Recipe(current_User) ?
    prelude(() =>
      Api.get_PreparationTime_PreparationTime_Recipes(self.props.entity, Thirty_PreparationTime_Recipe_page_index(self), Thirty_PreparationTime_Recipe_page_size(self), Thirty_PreparationTime_Recipe_search_query(self)).then(Recipes =>
...
export function load_relations_Thirty(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Thirty_PreparationTime_Recipe(self, false, self.props.current_User, 
        () => callback && callback())
}

export function set_size_Thirty(self:ThirtyContext, new_size:Utils.EntitySize) {
  self.props.set_size(new_size, () => {
    if (new_size == "fullscreen")
      self.props.push(Thirty_to_page(self.props.entity.Id))
  })
}
...
export function render_breadcrumb_Thirty(self:ThirtyContext) {
  return <div className="breadcrumb-thirty">Thirty</div>
}

export function render_menu_Thirty(self:ThirtyContext) {
  let state = self.state()
...
export function render_local_menu_Thirty(self:ThirtyContext) {
  let state = self.state()
          ...
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Recipe(self.props.current_User) ? null :
                  <div key={"PreparationTime_Recipe"} className={`local_menu_entry${self.props.shown_relation == "PreparationTime_Recipe" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Thirty_PreparationTime_Recipe(self,
                        false,
...
export function render_controls_Thirty(self:ThirtyContext) {
  return <div className="control">
    {self.props.allow_maximisation && self.props.set_size ? <a className={`"thirty button button--toggle ${self.props.size != 'preview' ? 'button--toggle--open' : ''}`}
          onClick={() => {
            set_size_Thirty(self, self.props.size == "preview" ? "large" : "preview")}
          }>
      </a> : null}
    {self.props.allow_fullscreen && self.props.set_size ? <a className="thirty button button--fullscreen"
        onClick={() => set_size_Thirty(self, self.props.size == "fullscreen" ? "large" : "fullscreen")}>
      </a> : null}
    {Permissions.can_delete_Thirty(self.props.current_User) && self.props.size == "fullscreen" ? <a className="button button--delete"
      onClick={() => confirm(i18next.t('Are you sure?')) &&
        Api.delete_Thirty(self.props.entity).then(() => self.props.force_reload(() => self.props.pop()))
      }>
    </a> : null}
    {self.props.size == "fullscreen" && self.props.pages_count > 0 ? <a className="thirty button button--close"
...
export function render_content_Thirty(self:ThirtyContext) {
  ...
  let content =
    Permissions.can_view_Thirty(self.props.current_User) ?
...
export function render_large_Thirty(self:ThirtyContext) {
  let state = self.state()
  let attributes:JSX.Element = null
  if (self.props.mode == "view" || !Permissions.can_edit_Thirty(self.props.current_User))
    attributes = (<div className="model__attributes">
...
export function render_Thirty_PreparationTime_Recipe(self:ThirtyContext, context:"presentation_structure"|"default") {
  ...
  return <div>
    ...
    { List.render_relation("thirty_preparationtime_recipe",
   "PreparationTime",
   "Recipe",
   "Recipes",
  ...
  (
      state.Recipe != "loading" ?
        ...
        state.Recipe,
      Thirty_PreparationTime_Recipe_page_index(self),
      Thirty_PreparationTime_Recipe_num_pages(self),
      new_page_index => {
          let state = self.state()
          state.Recipe != "loading" &&
...
export function render_relations_Thirty(self:ThirtyContext) {
  return <div className="relations">
      { render_Thirty_PreparationTime_Recipe(self, "default") }
      
    </div>
}

export function render_add_existing_Thirty_PreparationTime_Recipe(self:ThirtyContext) {
...
export function render_new_Thirty_PreparationTime_Recipe(self:ThirtyContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
                      ...
                      onClick={() =>
                          Api.create_linked_PreparationTime_PreparationTime_Recipes_Recipe(self.props.entity).then(e => {
                              ...
                              Api.update_Recipe(
                                ...
                                load_relation_Thirty_PreparationTime_Recipe(self, true, self.props.current_User, () =>
                                    self.setState({...self.state(), add_step_Recipe:"closed"})
                                  )
                                )
                          })
...
export function render_saving_animations_Thirty(self:ThirtyContext) {
...
export type ThirtyContext = {state:()=>ThirtyState, props:Utils.EntityComponentProps<Models.Thirty>, setState:(new_state:ThirtyState, callback?:()=>void) => void}

export type ThirtyState = {
    update_count:number
...
export class ThirtyComponent extends React.Component<Utils.EntityComponentProps<Models.Thirty>, ThirtyState> {
  constructor(props:Utils.EntityComponentProps<Models.Thirty>, context:any) {
    super(props, context)
  ...
  componentWillReceiveProps(new_props:Utils.EntityComponentProps<Models.Thirty>) {
  ...
  render() {
    ...
    return <div id={`Thirty_${this.props.entity.Id.toString()}_${this.state.update_count}`} className={`model thirty`}>
...
export let Thirty = (props:Utils.EntityComponentProps<Models.Thirty>) : JSX.Element =>
  <ThirtyComponent {...props} />

export let Thirty_to_page = (id:number) => {
  ...
  return Utils.scene_to_page<Models.Thirty>(can_edit, Thirty, Api.get_Thirty(id), Api.update_Thirty, "Thirty", "Thirty", `/Thirties/${id}`)
}

export let Thirty_to = (id:number, target_element_id:string, current_User:Models.User) => {
