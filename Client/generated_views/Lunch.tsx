...
export function Lunch_Lunch_Recipe_can_create(self:LunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanCreate
}
export function Lunch_Lunch_Recipe_can_delete(self:LunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanDelete
}
export function Lunch_Lunch_Recipe_page_index(self:LunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 0 : state.Recipe.PageIndex
}
export function Lunch_Lunch_Recipe_page_size(self:LunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 25 : state.Recipe.PageSize
}
export function Lunch_Lunch_Recipe_num_pages(self:LunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 1 : state.Recipe.NumPages
}

export function load_relation_Lunch_Lunch_Recipe(self:LunchContext, current_User:Models.User, callback?:()=>void) {
  Permissions.can_view_Recipe(current_User) ?
    Api.get_Lunch_Lunch_Recipes(self.props.entity, Lunch_Lunch_Recipe_page_index(self), Lunch_Lunch_Recipe_page_size(self)).then(Recipes =>
...
export function load_relations_Lunch(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Lunch_Lunch_Recipe(self, self.props.current_User, 
        () => callback && callback())
}
...
export function render_local_menu_Lunch(self:LunchContext) {
  ...
  return <div className="local-menu">
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Recipe(self.props.current_User) ? null :
                  <div key={"Lunch_Recipe"} className={`local_menu_entry${self.props.shown_relation == "Lunch_Recipe" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Lunch_Lunch_Recipe(self,
                        self.props.current_User, 
                        () => self.props.set_shown_relation("Lunch_Recipe"))
                    }>
                      {i18next.t('Lunch_Recipes')}
                    </a>
                  </div>
                }  
          </div>
...
export function render_Lunch_Lunch_Recipe(self:LunchContext, context:"presentation_structure"|"default") {
  if ((context == "default" && self.props.shown_relation != "all" && self.props.shown_relation != "Lunch_Recipe") || !Permissions.can_view_Recipe(self.props.current_User))
    return null
  let state = self.state()
  return <div>
    { List.render_relation("lunch_lunch_recipe",
   "Lunch",
   "Recipe",
   "Recipes",
  ...
  (
      ...
      Lunch_Lunch_Recipe_page_index(self),
      Lunch_Lunch_Recipe_num_pages(self),
      new_page_index => {
          let state = self.state()
            ...
            <div key={i_id}>
              {
                RecipeViews.Recipe({
                  ...
                  unlink: !Permissions.can_delete_Lunch_Recipe(self.props.current_User) ?
                    null
                    :
                    () => confirm(i18next.t('Are you sure?')) && Api.unlink_Lunch_Lunch_Recipes(self.props.entity, i.element).then(() =>
                      load_relation_Lunch_Lunch_Recipe(self, self.props.current_User))
                })
              }
            </div>
      ...
      () =>
        <div>
          {Permissions.can_create_Recipe(self.props.current_User) && Permissions.can_create_Lunch_Recipe(self.props.current_User) && Lunch_Lunch_Recipe_can_create(self) ? render_new_Lunch_Lunch_Recipe(self) : null}
          {Permissions.can_create_Lunch_Recipe(self.props.current_User) ? render_add_existing_Lunch_Lunch_Recipe(self) : null}
        </div>)
    }
...
export function render_relations_Lunch(self:LunchContext) {
  return <div className="relations">
      { render_Lunch_Lunch_Recipe(self, "default") }
      
    </div>
}

export function render_add_existing_Lunch_Lunch_Recipe(self:LunchContext) {
      ...
      <div className="button__actions">
        {
          ...
          React.createElement(List.AddToRelation,
            {
              relation_name:"lunch_lunch_recipe",
              source_name:"Lunch",
              target_name:"Recipe",
                  ...
                  <a className="group__button button button--existing"
                    onClick={() =>
                        self.setState({...self.state(), add_step_Recipe:"saving"}, () =>
                          Api.link_Lunch_Lunch_Recipes(self.props.entity, i).then(() =>
              ...
              get_items:[
                { name: "Recipe", get: async(i,s) => Api.get_unlinked_Lunch_Lunch_Recipes(self.props.entity, i, s) },
              ]
            })
        }
      </div>
...
export function render_new_Lunch_Lunch_Recipe(self:LunchContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
        <div className="new-recipe">
              <button 
                      ...
                      onClick={() =>
                          Api.create_linked_Lunch_Lunch_Recipes_Recipe(self.props.entity).then(e => {
                              e.length > 0 &&
                              Api.update_Recipe(
...
export let Lunch_to_page = (id:number) => {
  let can_edit = Utils.any_of([Permissions.can_edit_Lunch, Permissions.can_edit_Lunch_Recipe, Permissions.can_edit_Recipe])
