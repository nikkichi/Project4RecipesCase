...
export function Brunch_Brunch_Recipe_can_create(self:BrunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanCreate
}
export function Brunch_Brunch_Recipe_can_delete(self:BrunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? false : state.Recipe.CanDelete
}
export function Brunch_Brunch_Recipe_page_index(self:BrunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 0 : state.Recipe.PageIndex
}
export function Brunch_Brunch_Recipe_page_size(self:BrunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 25 : state.Recipe.PageSize
}
export function Brunch_Brunch_Recipe_num_pages(self:BrunchContext) {
  let state = self.state()
  return state.Recipe == "loading" ? 1 : state.Recipe.NumPages
}

export function load_relation_Brunch_Brunch_Recipe(self:BrunchContext, current_User:Models.User, callback?:()=>void) {
  Permissions.can_view_Recipe(current_User) ?
    Api.get_Brunch_Brunch_Recipes(self.props.entity, Brunch_Brunch_Recipe_page_index(self), Brunch_Brunch_Recipe_page_size(self)).then(Recipes =>
...
export function load_relations_Brunch(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Brunch_Brunch_Recipe(self, self.props.current_User, 
        () => callback && callback())
}
...
export function render_local_menu_Brunch(self:BrunchContext) {
  ...
  return <div className="local-menu">
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Recipe(self.props.current_User) ? null :
                  <div key={"Brunch_Recipe"} className={`local_menu_entry${self.props.shown_relation == "Brunch_Recipe" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Brunch_Brunch_Recipe(self,
                        self.props.current_User, 
                        () => self.props.set_shown_relation("Brunch_Recipe"))
                    }>
                      {i18next.t('Brunch_Recipes')}
                    </a>
                  </div>
                }  
          </div>
...
export function render_Brunch_Brunch_Recipe(self:BrunchContext, context:"presentation_structure"|"default") {
  if ((context == "default" && self.props.shown_relation != "all" && self.props.shown_relation != "Brunch_Recipe") || !Permissions.can_view_Recipe(self.props.current_User))
    return null
  let state = self.state()
  return <div>
    { List.render_relation("brunch_brunch_recipe",
   "Brunch",
   "Recipe",
   "Recipes",
  ...
  (
      ...
      Brunch_Brunch_Recipe_page_index(self),
      Brunch_Brunch_Recipe_num_pages(self),
      new_page_index => {
          let state = self.state()
            ...
            <div key={i_id}>
              {
                RecipeViews.Recipe({
                  ...
                  unlink: !Permissions.can_delete_Brunch_Recipe(self.props.current_User) ?
                    null
                    :
                    () => confirm(i18next.t('Are you sure?')) && Api.unlink_Brunch_Brunch_Recipes(self.props.entity, i.element).then(() =>
                      load_relation_Brunch_Brunch_Recipe(self, self.props.current_User))
                })
              }
            </div>
      ...
      () =>
        <div>
          {Permissions.can_create_Recipe(self.props.current_User) && Permissions.can_create_Brunch_Recipe(self.props.current_User) && Brunch_Brunch_Recipe_can_create(self) ? render_new_Brunch_Brunch_Recipe(self) : null}
          {Permissions.can_create_Brunch_Recipe(self.props.current_User) ? render_add_existing_Brunch_Brunch_Recipe(self) : null}
        </div>)
    }
...
export function render_relations_Brunch(self:BrunchContext) {
  return <div className="relations">
      { render_Brunch_Brunch_Recipe(self, "default") }
      
    </div>
}

export function render_add_existing_Brunch_Brunch_Recipe(self:BrunchContext) {
      ...
      <div className="button__actions">
        {
          ...
          React.createElement(List.AddToRelation,
            {
              relation_name:"brunch_brunch_recipe",
              source_name:"Brunch",
              target_name:"Recipe",
                  ...
                  <a className="group__button button button--existing"
                    onClick={() =>
                        self.setState({...self.state(), add_step_Recipe:"saving"}, () =>
                          Api.link_Brunch_Brunch_Recipes(self.props.entity, i).then(() =>
              ...
              get_items:[
                { name: "Recipe", get: async(i,s) => Api.get_unlinked_Brunch_Brunch_Recipes(self.props.entity, i, s) },
              ]
            })
        }
      </div>
...
export function render_new_Brunch_Brunch_Recipe(self:BrunchContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
        <div className="new-recipe">
              <button 
                      ...
                      onClick={() =>
                          Api.create_linked_Brunch_Brunch_Recipes_Recipe(self.props.entity).then(e => {
                              e.length > 0 &&
                              Api.update_Recipe(
...
export let Brunch_to_page = (id:number) => {
  let can_edit = Utils.any_of([Permissions.can_edit_Brunch, Permissions.can_edit_Brunch_Recipe, Permissions.can_edit_Recipe])
