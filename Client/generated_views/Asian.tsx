...
export function Asian_Cuisine_Recipe_can_create(self:AsianContext) {
  let state = self.state()
...
export function Asian_Cuisine_Recipe_can_delete(self:AsianContext) {
  let state = self.state()
...
export function Asian_Cuisine_Recipe_page_index(self:AsianContext) {
  let state = self.state()
...
export function Asian_Cuisine_Recipe_page_size(self:AsianContext) {
  let state = self.state()
...
export function Asian_Cuisine_Recipe_search_query(self:AsianContext) {
  let state = self.state()
...
export function Asian_Cuisine_Recipe_num_pages(self:AsianContext) {
  let state = self.state()
...
export function load_relation_Asian_Cuisine_Recipe(self:AsianContext, force_first_page:boolean, current_User:Models.User, callback?:()=>void) {
  let state = self.state()
  ...
  Permissions.can_view_Recipe(current_User) ?
    prelude(() =>
      Api.get_Cuisine_Cuisine_Recipes(self.props.entity, Asian_Cuisine_Recipe_page_index(self), Asian_Cuisine_Recipe_page_size(self), Asian_Cuisine_Recipe_search_query(self)).then(Recipes =>
        self.setState({...self.state(), update_count:self.state().update_count+1,
...
export function load_relations_Asian(self, current_User:Models.User, callback?:()=>void) {
  load_relation_Asian_Cuisine_Recipe(self, false, self.props.current_User, 
  ...
  return <div className="local-menu">
        ...
        {!Permissions.can_view_Recipe(self.props.current_User) ? null :
                  <div key={"Cuisine_Recipe"} className={`local_menu_entry${self.props.shown_relation == "Cuisine_Recipe" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Asian_Cuisine_Recipe(self,
                        false,
                        self.props.current_User, 
...
export function render_Asian_Cuisine_Recipe(self:AsianContext, context:"presentation_structure"|"default") {
  ...
  return <div>
    ...
    { List.render_relation("asian_cuisine_recipe",
   "Cuisine",
   "Recipe",
   "Recipes",
   self.props.nesting_depth > 0,
  ...
  (
      state.Recipe != "loading" ?
        ...
        state.Recipe,
      Asian_Cuisine_Recipe_page_index(self),
      Asian_Cuisine_Recipe_num_pages(self),
      new_page_index => {
          let state = self.state()
          state.Recipe != "loading" &&
          self.setState({...self.state(),
...
export function render_add_existing_Asian_Cuisine_Recipe(self:AsianContext) {
...
export function render_new_Asian_Cuisine_Recipe(self:AsianContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
                      ...
                      onClick={() =>
                          Api.create_linked_Cuisine_Cuisine_Recipes_Recipe(self.props.entity).then(e => {
                              ...
                              Api.update_Recipe(
                                ...
                                load_relation_Asian_Cuisine_Recipe(self, true, self.props.current_User, () =>
                                    self.setState({...self.state(), add_step_Recipe:"closed"})
                                  )
                                )
                          })
